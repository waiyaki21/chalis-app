    ...
    <div class="max-h-full px-2">
        ...
        <div :id="[classInfo.view_name]">
            <div :class="[classInfo.tabBody]" v-if="classInfo.tab1body">
                ...
                <div class="p-2 sm:p-2 w-full m-1 text-left mx-auto grid grid-cols-2 gap-2">
                    <UpdateProfileInformationForm
                        :must-verify-email="mustVerifyEmail"
                        :status="status"
                        class="col-span-1 rounded-md border-2 shadow-md border-cyan-500 overflow-hidden bg-cyan-400/10 dark:bg-cyan-400/10 h-fit p-2"
                    />

                    <UpdatePasswordForm class="col-span-1 rounded-md border-2 shadow-md border-cyan-500 overflow-hidden bg-cyan-400/10 dark:bg-cyan-400/10 h-fit p-2" />
                </div>
            </div>
            <div :class="[classInfo.tabBody]" v-if="classInfo.tab2body">
                <h2 class="text-red-600 dark:text-red-600 underline uppercase text-4xl my-1 mx-4">{{ classInfo.tab2name }}.</h2>
                <div class="p-2 sm:p-2 w-full m-1 text-left mx-auto">
                    <DeleteUserForm class="w-full rounded-md border-2 shadow-md border-rose-500 overflow-hidden bg-rose-400/10 dark:bg-rose-400/10 bg-rose-100 dark:bg-rose-800 p-2" />
                </div>
            </div>
        </div>
    </div>
