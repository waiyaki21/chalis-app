<template>
    <section class="fixed top-[1rem] right-5 flex-col z-[141] boldened overflow-y-scroll hidescroll max-h-[90%]" v-if="this.show">
        <TransitionGroup name="list" tag="div">
            <div v-for="flash in flashes" class="my-0.5 bg-transparent dark:bg-transparent pl-0.5 py-0.5 rounded-l-lg" :key="flash">
                <!-- button flash  -->
                <div :class="['flex-col gap-1 items-center p-2 md:py-4 border rounded-md shadow-md cursor-pointer w-[17rem] md:w-[22.5rem] animate__animated animate__slideInDown', flash.class]"
                    role="alert" v-if="flash.click" @click="hide(flash.id)">
                    <!-- header  -->
                    ...
                    <div class="inline-flex w-full justify-between gap-1">
                        <ActionButton :buttonClass="'dark'" :tooltipText="`Go to link`" :buttonText="`${flash.button}`" class="rounded-md shadow-md w-full" @handleClick="flash.urlLink ? $goTo(flash.link) : $downloadFile(flash.link)"> 
                            <right-icon class="mx-2 w-4 h-4 md:w-5 md:h-5 hover:rotate-180"></right-icon>
                        </ActionButton>
                        <StyleButton :buttonClass="'danger'" :tooltipText="`Close Notification`" :buttonText="`Close`" class="rounded-md shadow-md text-black flex-shrink-0" @handleClick="hide(flash.id)">
                            <times-icon class="m-1 w-4 h-4 md:w-5 md:h-5"></times-icon>
                        </StyleButton>
                    </div>
                </div>
                <!-- regular flash  -->
                <div :class="['flex items-center p-2 py-3 border rounded-md shadow-md cursor-pointer w-[17rem] md:w-[22.5rem] animate__animated animate__slideInDown', flash.class]" role="alert" v-if="!flash.click" @click="hide(flash.id)">
                    <!-- icons  -->
                    <component
                        :is     = "flash.icon"
                        :class  = "flash.iconClass"
