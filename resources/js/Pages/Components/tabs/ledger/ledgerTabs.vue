<template>
    <div class="p-2 flex-col space-y-2 font-boldened">
        <!-- <h3 :class="[classStyle.headerMain]" @click="info.viewShow = !info.viewShow">
            <span>
            ...
            <section class="grid grid-cols-1 md:grid-cols-5 gap-1" v-if="!info.noCycles">
                <div class="m-1 p-2 py-4 rounded-lg bg-gray-50 dark:bg-gray-800 border-2 border-cyan-600 dark:border-cyan-600 h-fit col-span-2">
                    <ol class="relative border-s border-gray-200 dark:border-gray-700 py-2 mx-2">                  
                        <li class="mb-4 ms-6">
                            <span :class="[classStyle.spanClass]">
                                <svg :class="[classStyle.svgClass]" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
                        ...
                        <li class="mb-4 ms-6">
                            ...
                            <section class="inline-flex w-full space-x-2 my-2">
                                <div class="w-full flex-col space-y">
                                    ...
                                    <select id="start" v-model="info.start" name="start" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 w-full">
                                        <!-- <option :value="firstMonth.id" class="uppercase my-1">{{ firstMonth.month }} {{ firstMonth.year }}</option> -->
                                ...
                                <div class="w-full flex-col space-y">
                                    ...
                                    <select id="end" v-model="info.end" name="end" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 w-full">
                                        <!-- <option :value="lastMonth.id" class="uppercase my-1">{{ lastMonth.month }} {{ lastMonth.year }}</option> -->
                                        <option v-for="(month, index) in info.endMonths" :value="month.id" class="uppercase my-1" @click="checkEnd(month)">{{ info.endMonths.length - index }}. {{ month.month }} {{ month.year }}</option>
                                    </select>
                                </div>
                            </section>
                            <section class="w-full inline-flex my-1">
                                ...
                                <a :href="'/download/ledger/'+ info.start + '/' + info.end" :class="[classStyle.btnSelect]" v-if="info.start != '' && info.end != ''">
                ...
                <div class="m-1 p-2 py-4 rounded-lg bg-gray-50 dark:bg-gray-800 border-2 border-cyan-600 dark:border-cyan-600 h-fit col-span-3">
                    <ol class="relative border-s border-gray-200 dark:border-gray-700 p-2 mx-2">
                        <li class="mb-4 ms-6">
                            ...
                            <h3 :class="[classStyle.headerOption]" @click="info.yearShow = !info.yearShow">
                                <span>
                                    <u>Ledgers By Year</u>
                            ...
                            <section class="flex-col w-full p-1 space-y-2 rounded-lg bg-gray-50 dark:bg-cyan-600/20 border-2 border-cyan-600 dark:border-cyan-600 overflow-y-scroll h-auto max-h-60" v-if="info.yearShow">
                        ...
                        <li class="mb-1 ms-6">
                            ...
                            @click="info.monthShow = !info.monthShow">
                                <span>
                            ...
                            <section class="flex-col w-full p-1 space-y-2 rounded-lg bg-gray-50 dark:bg-cyan-600/20 border-2 border-cyan-600 dark:border-cyan-600 overflow-y-scroll h-fit max-h-[8.5rem]" v-if="info.monthShow">
...
<script setup>
    ...
    const classStyle = reactive({
        ...
        btnDanger: 'inline-flex items-center px-4 py-2 text-base font-medium text-gray-900 bg-white border-2 border-gray-200 rounded-lg hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring focus:outline-none focus:ring-rose-200 focus:text-blue-700 dark:bg-rose-400 dark:text-gray-800 dark:border-gray-900 dark:hover:text-black dark:hover:bg-rose-700 dark:focus:ring-rose-700 uppercase w-full justify-between my-2 cursor-not-allowed',
        selectBtn: '',
    ...
    const emit = defineEmits(['flash-show'])

    onBeforeMount(() => setInfo())
    ...
    function setInfo() {
        ...
        if (info.end == '') {
            classStyle.selectBtn = classStyle.btnDanger;
        } else {
            classStyle.selectBtn = classStyle.btnSelect;
        }
    }
    ...
    function months() {
        // info.monthShow = !info.monthShow
        axios.get('/api/getCyclesInfo')
            .then(
                ({ data }) => {
                    // console.log(data)
                    props.cycleinfo[1] = data[1]
                    info.endMonths = props.cycleinfo[1]
                    info.lastMonth   = data[3]
                    info.firstMonth  = data[2]
                }
            )
    }
