            ...
            <div class="px-0.5 py-1 font-boldened">
                ...
                <form @submit.prevent="submitEdit" class="p-2 flex-col grid md:grid-cols-2 grid-cols-1 gap-1">
                    <div class="col-span-1 md:col-span-2">
                        ...
                        <TextInput id="name" type="text" v-model="formedit.name" :placeholder="formedit.name"
                            required autofocus />
...
<script setup>
    ...
    function submitEdit() {
        ...
        axios.put('/update/cycleExpenseName/' + classInfo.modalData.id, formedit)
            .then(({ data }) => {
                clearFields();
                closeModal();

                emit('hide');
                const message = `Expense Type ${props.info.name} Updated!`;
                const type = 'update';
