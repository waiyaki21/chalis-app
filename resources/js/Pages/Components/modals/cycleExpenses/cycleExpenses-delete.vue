        ...
        <section class="p-2 font-boldened">
            ...
            <section class="flex w-full justify-start my-1">
                <h3 class="font-normal sm:text-sm md:text-md text-red-800 dark:text-red-600 leading-tight uppercase underline py-1 w-full decoration-red-900">
                    Are you sure you want to delete Cycle Expense: {{ info.name }}?
                </h3>
            </section>
            ...
            <section class="w-full justify-between space-x-1 grid grid-cols-5 gap-0.5">
                <ActionButton :class="'col-span-4'" buttonClass='submitDanger' @handleClick="deleteInfo" :tooltipText="`Delete Cycle Expense: ${classInfo.deleteData.name}`" :buttonText="`Delete Cycle Expense.`">
...
<script setup>
    ...
    const classInfo = reactive({
        ...
        deleteCycleURL: '/delete/cycle/cycleExpense/',
    ...
    function deleteInfo() {
        ...
        router.delete(url, {
            ...
            onSuccess: () => [
                // flashMessage 
                message   = 'Cycle Expense '+ name +' Deleted!',
                type      = 'danger',
